...
;(async () => {
  ...
  function createContent(key: string, value: string | Record<string, string>, currentPath: string) {
    if (key === "TODO") {
    ...
    if (typeof value === "string") fs.writeFileSync(path.join(currentPath, key), value.replace(/{{name}}/g, projectName))
      ...
      for (const k in value) {
        createContent(k, value[k], newPath)
      }
    }
  }

  for (const k in CONTENTS[projectType][projectLang]) createContent(k, CONTENTS[projectType][projectLang][k as keyof (typeof CONTENTS)["discord" | "fast"]["javascript" | "typescript"]], fullPath)
})()
